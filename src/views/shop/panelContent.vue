      ...
      <el-col :span="24" class="warp-main">
        ...
        <el-col :span="24">
          <el-col :span="24" style="margin-bottom: 5px">
            <el-select v-model="tree" @change="onSelected($event, tree)">
              <el-option v-for="item in panelTree" :key="item.id" :label="item.title" :value="item.id" />
            </el-select>
          ...
          <el-table v-loading="loading" :data="dataList" border stripe highlight-current-row style="width: 100%;" @selection-change="selsChange">
            ...
            <el-table-column prop="picUrl" label="缩略图" width="80" align="center">
              <template slot-scope="scope">
                <img :src="scope.row.picUrl" size="medium" class="img">
              </template>
            </el-table-column>
        ...
        <!-- 商品列表对话框 -->
        <el-dialog :close-on-click-modal="false" :visible.sync="productVisible" title="商品列表" style="margin-bottom: 20px;">
...
export default{
  ...
  methods: {
    ...
    getPanelTree() {
      var that = this
      API.mainTree().then(function(result) {
    ...
    onSelected(event, item) {
      for (var i = 0; i < this.panelTree.length; i++) {
        if (this.panelTree[i].id === item) {
          this.panelName = this.panelTree[i].title
        }
      }
      this.selectId = item
      this.handleSearch()
